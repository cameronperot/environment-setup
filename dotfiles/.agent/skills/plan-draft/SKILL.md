...
3. Structure sequential phases:
   ...
   - Define explicit entry criteria, dependencies, and exit phase gates for each phase.
...
4. Define verifiable tasks:
   ...
   - State the narrowest, honest verification step (e.g. exact command, query, or automated test) for every task.
...
6. Present the plan:
   ...
   - Output the plan to the conversation or write it to a requested plan file path.
...
- No duration, time estimates, or sprint allocations are present.
