...
def test_unreadable_gitconfig_is_reported_when_adding(tmp_path, caplog):
    env = build_env(
        tmp_path,
        "include: [.gitconfig]\n",
        home_files={".gitconfig": "seed"},
    )
    (env.home / ".gitconfig").write_bytes(b"[user]\n\xff\xfe\n")

    with caplog.at_level(logging.ERROR, logger="sync_dotfiles"):
        exit_code = make_syncer(env).run()

    assert exit_code == 1
    assert ".gitconfig: cannot read file for scrubbing" in caplog.text
    assert not (env.dotfiles / ".gitconfig").exists()


def test_unreadable_gitconfig_is_reported_when_comparing(tmp_path, caplog):
    env = build_env(
        tmp_path,
        "include: [.gitconfig]\n",
        home_files={".gitconfig": "seed"},
    ...
    assert exit_code == 1
    assert ".gitconfig: cannot compare with repo copy" in caplog.text
    assert snapshot(env.dotfiles) == before_repo


def test_mode_only_change_recopies_the_redacted_gitconfig(tmp_path):
    redacted = "[user]\n    signingkey = (redacted)\n"
    env = build_env(
        tmp_path,
        "include: [.gitconfig]\n",
        home_files={".gitconfig": redacted},
        repo_files={".gitconfig": redacted},
    )
    os.chmod(env.home / ".gitconfig", 0o755)

    assert make_syncer(env).run() == 0
    assert (env.dotfiles / ".gitconfig").stat().st_mode & 0o7777 == 0o755
